...
class ViewController: UIViewController {
    ...
    // 수식 문자열을 계산해서 결과 문자열 반환 (크래시 방지)
    private func calculateResult(_ expression: String) -> String? {
        // 연산자로 끝나면 마지막 연산자 제거하여 안전 계산
        var expr = expression
        ...
        if let number = nsExpression.expressionValue(with: nil, context: nil) as? NSNumber {
            let intValue = Int(trunc(number.doubleValue))
    ...
    private func buttonClicked(_ sender: UIButton) {
        ...
        case "=":
            // 수식이 연산자로 끝나면 제거 후 계산
            while let last = currentText.last, "+-*/".contains(last) {
                currentText.removeLast()
            }
            if let result = calculateResult(currentText) {
                currentText = result
            } else {
                currentText = "Error"
            }
        ...
        default:  // +, -, *, /
            // 연산자가 연속으로 오지 않도록 간단 방어
            if let last = currentText.last, "+-*/".contains(last) {
                currentText.removeLast()
            }
